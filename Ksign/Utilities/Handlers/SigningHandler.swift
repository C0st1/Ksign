...
final class SigningHandler: NSObject {
        ...
        await withCheckedContinuation { (continuation: CheckedContinuation<Void, Never>) in
            ...
            Storage.shared.addSigned(
                uuid: _uuid,
                source: (_app as? Signed)?.source ?? (_app as? Imported)?.source,
                ...
                // Clear any stale update entry for this app
                if let bundleId = bundle?.bundleIdentifier {
                    UpdateManager.shared.clearUpdate(for: bundleId)
                }
                // Track the source URL for future update checks
                if let bundleId = bundle?.bundleIdentifier,
                   let sourceURL = (self._app as? Signed)?.source ?? (self._app as? Imported)?.source {
                    SourceTracker.shared.recordSource(bundleId: bundleId, sourceURL: sourceURL)
                }
                continuation.resume()
            }
                }
