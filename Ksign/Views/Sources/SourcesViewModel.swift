...
final class SourcesViewModel: ObservableObject {
        ...
        func fetchSources(_ sources: FetchedResults<AltSource>, refresh: Bool = false, batchSize: Int = 4) async {
                ...
                // Only clear sources on initial fetch, not on refresh.
                // Clearing during refresh causes the table view to lose its
                // scroll position and flash the empty-state placeholder.
                if !refresh {
                        await MainActor.run {
                                self.sources = [:]
                        }
